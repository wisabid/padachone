...
const Layout = ({country, region, place, method, school, pdate, startup}) => {
    ...
    if (data && data.data && data.data.meta && data.code === 200) {
        ...
        return (
            <>
            <h5>{data.data || (data.error === 'Failed to fetch')?<SiteMessage type="info" message={`Site is down for maintenance! Please try after sometime.`} action="Refresh" />:data.error}</h5> 
