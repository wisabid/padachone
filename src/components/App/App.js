...
function App() {
  ...
  useEffect(() => {
    if (forceTrigger.target === FT_PRAYER) {
      setState({
        ...state,
        method: localStorage.getItem("padachone:method")
          ? parseInt(localStorage.getItem("padachone:method"))
          : 8,
        school: localStorage.getItem("padachone:school")
          ? parseInt(localStorage.getItem("padachone:school"))
          : 0
      });
    }
  }, [forceTrigger]);
  ...
  useEffect(() => {
    ...
    if (msgbroadcast) {
      ...
      setMsg(() => {
        ...
        return [
          true,
          msgbroadcast.data[PRISMIC_MSG_BROADCAST_DOC].edges[0].node
        ];
      });
    }
    // Logic for displaying Messages end here
    const padachon_lsfind = Object.keys(localStorage).filter(
      key =>
        key.startsWith("padachone:") &&
        key !== "padachone:region" &&
        key !== "padachone:country" &&
        key !== "padachone:place" &&
        key !== "padachone:method" &&
      ...
      <div className="App">
        ...
        >
          <ErrorBoundary>
            <CookieConsent
              location="bottom"
              style={{ background: "#29b6f6", marginBottom: "30px" }}
            ...
            {finished && page === "Home" && (
              <Layout
                ...
                method={method}
                school={school}
                startup={resetstate => handlefinished(resetstate)}
              />
            )}
            {modal.show && modal.name === "Subscribe" && (
              <Subscribe modal={modal} setModal={setModal} />
            )}
            {modal.show && modal.name === "Finetune" && (
              <Finetune
                modal={modal}
                setModal={setModal}
                method={method}
            ...
            {modal.show && modal.name === "ConfirmAction" && (
              <ConfirmAction
                modal={modal}
                setModal={setModal}
