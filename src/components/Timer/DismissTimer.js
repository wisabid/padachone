...
import {useRenderCounts, useCmsAsset, useWhatsapplogger} from '../../hooks/api-hooks';
...
const DismissTimer  = ({dismissMsg, setdismissMsg, anchorEl, setAnchorEl, timerdisplay, setTimerdisplay}) => {    
  ...
  const [log, setLogs] = useWhatsapplogger({});
    ...
    const onClose1 = (event) => {
        ...
        setLogs({
            action: "Dismiss",
            message: `just attempted for dismissing the timer`
        });
    ...
    return (
      <>
        ...
        >
          {dismissMsg[0] === "Dismiss" ? dismissMsg[0] : null}
          {dismissMsg[0] !== "Dismiss" && (
            <img src={dismissMsg[1]} width="60" height="60" />
          )}
        </Button>
        <Popover
          id={id}
